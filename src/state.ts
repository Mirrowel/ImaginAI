...
// src/state.ts
import type { Scenario, AdventureTurn, Card, View, NewScenarioScaffold, ActionType, Adventure, GlobalSettings, ResponseHandlingStrategy, ScenarioSnapshot, EditorContext, AvailableTextModel } from './types';
...
// export let isEditingDefaultScenarioTemplate: boolean = false; // REMOVED: Flag for editing the default template

export let editingAdventure: Adventure | null = null; // Potentially redundant if currentEditorContext for adventure is used well
export let isLoadingAI: boolean = false;
export let currentCardTypeForEditor: string = 'character'; // Used in card form within editor
export let currentPlayerActionType: ActionType = 'do';
export let isPlayerActionInputVisible: boolean = false; // For gameplay action textarea
export let editingTurnId: string | null = null;

// Card Editing States
export let editingAdventureDetailsCardId: string | null = null; // For inline editing of cards in adventure details
export let showAddAdventureCardForm: boolean = false; // For toggling add card form in adventure details
export let editingScenarioEditorCardId: string | null = null; // For editing cards in the scenario editor
export let showAddScenarioCardForm: boolean = false; // For toggling add card form in scenario editor
...
export let scenarioEditorCardFilterType: string = ""; // Empty string for "All Types"
export let scenarioEditorCardDisplayType: 'grid' | 'list' | 'compact' = 'grid';


// Global User Settings
export let globalSettingsVisible: boolean = false;
export let responseHandlingStrategy: ResponseHandlingStrategy = 'truncate'; // Default
export let allowAiThinking: boolean = false; // Default updated to false
export let globalMaxOutputTokens: number = 200; // Default
export let selectedModel: AvailableTextModel = 'gemma-3-27b-it'; // Default model updated
...
export let currentGameplaySidebarTab: 'plot' | 'cards' | 'info' = 'plot'; // New state for gameplay sidebar tabs
export let isGameplaySidebarVisible: boolean = true; // Default to visible for easier dev/review
...
// REMOVED: setIsEditingDefaultScenarioTemplate
// export function setIsEditingDefaultScenarioTemplate(isEditing: boolean) { 
//     isEditingDefaultScenarioTemplate = isEditing;
// }
...
export function updateGlobalSettings(settings: Partial<GlobalSettings>) {
    ...
    if (settings.globalMaxOutputTokens !== undefined) {
        ...
        if (Number.isFinite(tokens) && tokens >= 50 && tokens <= 800) { // Widened max for global
...
export function setCurrentGameplaySidebarTab(tab: 'plot' | 'cards' | 'info') {
