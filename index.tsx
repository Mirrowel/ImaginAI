...
if (API_KEY && ai) {
    ...
    // This input is added to the DOM by renderScenarioList
    // We attach the listener here to ensure it's available globally once the app initializes
    // and scenario list is the first view.
    // A more robust way might be to attach it when scenarioListRenderer actually adds the input.
    // However, since it's a hidden input controlled by a button, this global attachment is acceptable.
    // Note: If this listener were specific to when the *button* is clicked, it'd be in scenarioListRenderer.
    // But this listens to the *file input itself* which is always 'there' (though hidden).
    // This specific handler needs to be called when the file input changes.
