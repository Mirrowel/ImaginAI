...
import { SkeletonBlock, SkeletonList, EmptyState } from "../primitives/LoadingStates";
...
export function AdminSettings() {
  ...
  return <section className="stack"><header className="section-heading"><h1>Admin Settings</h1><p className="muted">Global models appear in every user's gameplay picker without exposing credentials.</p></header><Panel title="Diagnostics">{diagnostics.isLoading ? <SkeletonBlock lines={4} /> : <div className="diagnostics-grid">{Object.entries(diagnostics.data ?? {}).map(([key, value]) => <div className="subcard" key={key}><strong>{value}</strong><span>{key}</span></div>)}</div>}</Panel><Panel title="Recent Usage">{usage.isLoading ? <SkeletonList count={3} /> : usage.data?.items?.length === 0 ? <EmptyState message="No usage recorded yet." /> : <div className="list">{usage.data?.items.map((row) => <div className="list-item" key={row.id}><strong>{row.totalTokens} tokens</strong><span>{row.provider} · {row.modelId} · {row.createdAt}</span></div>)}</div>}</Panel><Panel title="Global Provider"><form className="stack" onSubmit={submitProvider}><input name="name" placeholder="Display name" /><input name="providerType" placeholder="openrouter/openai/gemini/custom" /><input name="protocol" defaultValue="openai_chat_completions" /><input name="baseUrl" placeholder="Optional base URL" /><input name="apiKey" placeholder="Write-only platform API key" type="password" /><button>Create Global Provider</button></form></Panel><Panel title="Global Model"><ModelForm providers={providers.data?.items ?? []} onSubmit={submitModel} /></Panel><Panel title="Global Resources">{providers.isLoading || models.isLoading ? <SkeletonList count={2} /> : <div className="grid two"><div>{providers.data?.items?.length === 0 ? <EmptyState message="No global providers." /> : providers.data?.items.map((provider) => <ProviderRow key={provider.id} provider={provider} queryKey="adminProviders" admin />)}</div><div>{models.data?.items?.length === 0 ? <EmptyState message="No global models." /> : models.data?.items.map((model, index) => <ModelRow key={model.id} model={model} models={models.data?.items ?? []} index={index} queryKey="adminModels" admin />)}</div></div>}</Panel></section>;
}
